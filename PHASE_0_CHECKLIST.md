...
### 2. 모노레포 의존성 설치
- [ ] `pnpm install` 성공 (루트에서 실행)
- [ ] workspace 패키지 간 참조 정상 (`@tripcart/*` import 확인)

### 3. apps/web (Next.js 16)
- [ ] `pnpm --filter @tripcart/web dev` 실행 성공
- [ ] http://localhost:3000 접속 — Hello World 화면 확인
- [ ] 컬러 팔레트 4개 박스 (primary / plum / coral / gold) 정상 렌더링
...
- [ ] http://localhost:8000/health 응답 확인:
  ...
  ```
- [ ] `uv run pytest tests/ -v` 통과 (3개 테스트)

### 6. CI 파이프라인
- [ ] GitHub repo 연결 후 push — CI 자동 실행
- [ ] `pnpm lint` 통과
- [ ] `pnpm typecheck` 통과
