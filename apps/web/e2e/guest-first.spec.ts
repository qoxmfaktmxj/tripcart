...
test('게스트 저장과 초안 플랜은 로그인 후 계정으로 이어진다', async ({ page }, testInfo) => {
  ...
  await expect(
    page.getByText('카페·맛집·명소를 담고 — 출발 시간을 정하면 영업시간과 동선까지 맞춰 정렬됩니다.'),
  ).toBeVisible()

  const cartButton = page.getByRole('button', { name: /장바구니/ })
  await cartButton.click()
  const cartDialog = page.getByRole('dialog', { name: '장바구니' })
  await expect(cartDialog).toBeVisible()
  await expect(cartDialog).toHaveAttribute('aria-modal', 'true')
  ...
  await firstCard.getByRole('button', { name: '담기' }).click()
  ...
  const planTitle = `게플-${Date.now().toString().slice(-4)}`
  await page.goto('/plans')
  await page.getByRole('button', { name: '새 계획 만들기' }).click()
  await page.getByLabel('제목').fill(planTitle)
  await page.getByLabel('출발 일시').fill('2026-04-20T09:30')
  ...
  await page.getByRole('button', { name: '새 계획 만들기' }).click()
  await page.getByRole('link', { name: '로그인 후 계정으로 가져오기' }).click()
  ...
  await page.getByLabel('비밀번호 (6자 이상)').fill('tripcart123!')
  ...
  await page.goto('/plans')
  await page.getByRole('link', { name: new RegExp(planTitle) }).click()
  await expect(page.getByRole('heading', { name: '나의 여행 계획' })).toBeVisible()
  await expect(page.getByText(planTitle)).toBeVisible()
  await page.screenshot({
...
test('게스트 상태는 새로고침 후에도 브라우저에 유지된다', async ({ page }, testInfo) => {
  ...
  const planTitle = `유지-${Date.now().toString().slice(-4)}`
  await page.goto('/plans')
  await page.getByRole('button', { name: '새 계획 만들기' }).click()
  await page.getByLabel('제목').fill(planTitle)
  await page.getByLabel('출발 일시').fill('2026-04-21T10:15')
  await page.getByRole('button', { name: '생성' }).click()
  await expect(page.getByText(planTitle)).toBeVisible()
