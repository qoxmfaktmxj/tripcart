...
| Phase 1 | 제품 코어 | auth, places, saved places, plans CRUD | 구현됨; audit hardening pending |
| Phase 1.5 | guest-first trial | 공개 랜딩, guest 저장/초안, 로그인 이관 | 구현됨; QA/docs alignment pending |
| Phase 2 | 최적화 연동 | optimizer, alternatives, share/import | Supabase RLS smoke 후 진행 |
...
### 2026-04-15 audit 해소 상태
- SECURITY DEFINER RPC의 caller-supplied `p_user_id` 경계 hardening 완료
- `link_only` 공유 일정 직접 select 차단 및 share_code 검증 RPC 경로 추가
- `packages/types`, route handler response shape, optimizer DTO를 `API_CONTRACT_v0.2.md` 방향으로 정렬
- optimizer internal token 검증과 prod docs/redoc 비활성화 정책 추가
- `pnpm audit --prod --audit-level high`와 optimizer ruff 통과
- 남은 차단: Phase 2 실제 share/import/execution endpoint 구현
...
- 한국어 UI 깨짐 없음
- 모바일/데스크톱 랜딩 레이아웃 정상
- P0/P1 보안 회귀: owner mismatch, share visibility, optimizer internal auth
- 전체 품질 게이트: `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, web e2e, optimizer pytest, optimizer ruff, production dependency audit
