...
- secret scan
- Supabase RLS owner mismatch tests
...
- `pnpm test`
- `pnpm build`
- `pnpm --filter @tripcart/web e2e` (Playwright 2개)
- `uv run --extra dev pytest` (optimizer 8개)
- `uv run --extra dev ruff check .`
- `pnpm audit --prod --audit-level high` (high 취약점 없음)
- local Supabase `rpc_share_security_smoke.sql` (owner mismatch / `link_only` RLS)

### 미충족 / 수동 확인
- `.env.local` hosted secret 여부와 rotation 필요성 수동 확인 필요
...
- 문서 동기화됨
- GS1/GS2/GS3 를 깨지 않음
- 위 `required` 게이트가 모두 통과하거나, 실패 항목을 릴리스 차단 리스크로 명시함
